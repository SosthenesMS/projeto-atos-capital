...
public class Labirinto02 {
    ...
    public static void main(String[] args) {
        lerLabirinto("D:/ArquivosPessoais/Projects/projeto-atos-capital/Teste VSCode - VAGA 01/PROVA VAGA 1/Avaliação técnica - Atos Capital/template-solution-java/Template-Java/src/entrada-labirinto.txt");
        ...
        if (encontrarSaida()) {
            ...
            for (String passo : resultado) {
                System.out.println(passo);
            }

            gerarArquivoSaida("D:/ArquivosPessoais/Projects/projeto-atos-capital/Teste VSCode - VAGA 01/PROVA VAGA 1/Avaliação técnica - Atos Capital/template-solution-java/Template-Java/src/saida-entrada-labirinto.txt");
        ...
        while (!pilha.isEmpty()) {
            ...
            for (int i = 0; i < MOVIMENTOS_LINHA.length; i++) {
                ...
                if (posicaoValida(novaLinha, novaColuna)) {
                    ...
                    passos.append(movimento).append(" [").append(novaLinha).append(", ").append(novaColuna).append("]\n");
                    resultado.add(movimento + " [" + novaLinha + ", " + novaColuna + "]");
            ...
            if (!movimentoPossivel) {
                ...
                passos.setLength(passos.length() - 12);

                // Adicionar o movimento de retorno aos passos
                char movimentoRetorno = getMovimentoChar(indiceMovimento);
                passos.append(movimentoRetorno).append(" [").append(posicaoAnterior.getLinha()).append(", ")
                        .append(posicaoAnterior.getColuna()).append("]\n");
            }
        }
    ...
    private static void gerarArquivoSaida(String arquivo) {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter(arquivo))) {
            bw.write("Passos percorridos:\n");
            bw.write(passos.toString());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
