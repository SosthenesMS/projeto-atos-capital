...
public class DesafioDoLabirintoBk02 {
	...
	public static void main(String[] args) {
		...
		while (!achouSaida && iteracoes < maxIteracoes) {
			...
			if (lAtual > 0 && labirinto[lAtual - 1][cAtual].equals("0")) {
				...
				} else if (cAtual > 0 && labirinto[lAtual][cAtual - 1].equals("0")) {
				...
				} else if (cAtual < extremidadeColuna && labirinto[lAtual][cAtual + 1].equals("0")) {
				...
				} else if (lAtual < extremidadeLinha && labirinto[lAtual + 1][cAtual].equals("0")) {
				...
				if (!moveu) {
					...
					// Verifica se não houve movimento nesta iteração
					resultado.remove(resultado.size() - 1);
				...
				if (proximaPosicaoVisitada) {
					// Não é possível se movimentar para a próxima posição não visitada
					// Verificar se há posições anteriores que ainda não foram visitadas
					boolean encontrouPosicaoNaoVisitada = false;
					int posicaoAnterior = resultado.size() - 2;
			
					while (!encontrouPosicaoNaoVisitada && posicaoAnterior >= 0) {
						String[] posicao = resultado.get(posicaoAnterior).split(" ");
						int linhaPosicao = Integer.parseInt(posicao[1].replaceAll("[^0-9]", ""));
						int colunaPosicao = Integer.parseInt(posicao[2].substring(0, posicao[2].length() - 1));
			
						if (!visitados[linhaPosicao - 1][colunaPosicao - 1]) {
							encontrouPosicaoNaoVisitada = true;
							lAtual = linhaPosicao - 1;
							cAtual = colunaPosicao - 1;
						} else {
							resultado.remove(posicaoAnterior);
							posicaoAnterior--;
						}
					}
			
					if (!encontrouPosicaoNaoVisitada) {
						// Não há mais movimentos possíveis e não encontrou a saída
						System.out.println("Não foi possível encontrar a saída do labirinto");
						return;
					}
