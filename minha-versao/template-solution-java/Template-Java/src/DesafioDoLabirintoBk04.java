...
public class DesafioDoLabirintoBk04 {
	...
	public static void main(String[] args) {
		...
		while (!achouSaida && iteracoes < maxIteracoes) {
			...
			boolean moverParaCima = lAtual > 0 && labirinto[lAtual - 1][cAtual].equals(CAMINHO) && !up;
			boolean moverParaEsquerda = cAtual > 0 && labirinto[lAtual][cAtual - 1].equals(CAMINHO) && !left;
			boolean moverParaDireita = cAtual < extremidadeColuna && labirinto[lAtual][cAtual + 1].equals(CAMINHO) && !right;
			boolean moverParaBaixo = lAtual < extremidadeLinha && labirinto[lAtual + 1][cAtual].equals(CAMINHO) && !down;
			...
			if (moverParaCima) {
				...
				// 	resultado.remove(resultado.size() - 1); // Tem que voltar para a posição anterior

				// 	if (resultado.isEmpty()) {
						
				// 		System.out.println("Não foi possível encontrar a saída do labirinto"); // Não há mais movimentos possíveis e não encontrou a saída
				// 		return;
				// 	}
				...
				//historicoDeMovimentos.add(ultimoMovimento +" [" + (lAtual + 1) + ", " + (cAtual + 1) + "]");
				...
				//  System.out.println("nova linha = " + novaLinha + " nova coluna = " + novaColuna);
		...
		// resultado.remove(ultimaPosicao);
		//ultimaPosicao = resultado.size() - 1;
		//System.out.println("Resultado da ultima posição" + resultado.get(ultimaPosicao));
